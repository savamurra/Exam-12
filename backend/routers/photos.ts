...
photoRouter.post('/', imagesUpload.single("image"), auth, async (req, res, next) => {
...
photoRouter.delete("/:id", auth, permit('admin', 'user'), async (req, res, next) => {
    const expressReq = req as RequestWithUser;
        ...
        if (user.role === "admin") {
            ...
            if (currentPhoto) {
                if (currentPhoto.user.toString() !== user._id.toString()) {
                    res.status(403).send({ error: "You can not delete someone else's photo" });
                    return;
                } else  {
                    const photo = await Photo.findByIdAndDelete(id);
